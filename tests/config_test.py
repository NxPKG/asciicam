...
import asciicam.config as cfg
from asciicam.config import Config
...
def test_default_api_url() -> None:
    config = create_config("")
    assert config.api_url == "https://asciicam.org"
...
def test_api_url_when_override_set() -> None:
    config = create_config(
        "[api]\nurl = http://the/url", {"ASCIICAM_API_URL": "http://the/url2"}
    )
