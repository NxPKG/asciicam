...
DEFAULT_API_URL: str = "https://asciicam.org"
...
class Config:
    ...
    def upgrade(self) -> None:
        ...
        if self.env.get("ASCIICAM_API_TOKEN"):
            raise ConfigError(
                "ASCIICAM_API_TOKEN variable is no longer supported"
                ", please use ASCIICAM_INSTALL_ID instead"
            )
    ...
    def install_id(self) -> str:
        id_ = self.env.get("ASCIICAM_INSTALL_ID") or self.__read_install_id()
...
def get_config_home(env: Any = None) -> Any:
    if env is None:
        env = os.environ
    env_asciicam_config_home = env.get("ASCIICAM_CONFIG_HOME")
    env_xdg_config_home = env.get("XDG_CONFIG_HOME")
    env_home = env.get("HOME")

    config_home: Optional[str] = None

    if env_asciicam_config_home:
        config_home = env_asciicam_config_home
    elif env_xdg_config_home:
        config_home = path.join(env_xdg_config_home, "asciicam")
    elif env_home:
        if path.isfile(path.join(env_home, ".asciicam", "config")):
            # location for versions < 1.1
            config_home = path.join(env_home, ".asciicam")
        else:
            config_home = path.join(env_home, ".config", "asciicam")
    else:
        raise Exception(
            "need $HOME or $XDG_CONFIG_HOME or $ASCIICAM_CONFIG_HOME"
        )
