...
* ASCIICAM_REC env var is back (thanks @landonb / Landon Bouma!)
* Terminal answerbacks (CSI 6 n) in `asciicam cat` are now hidden (thanks @djpohly / Devin J. Pohly!)
...
* Possibility of programatic recording with `asciicam.record_asciicast` function
* Uses new JSON response format added recently to asciicam-server
...
* Fixed location of config dir (you can `mv ~/.asciicam ~/.config/asciicam`)
...
* ability to live-stream via UNIX pipe: `asciicam rec unix.pipe` + `asciicam play unix.pipe` in second terminal tab/window
* optional stdin recording (`asciicam rec --stdin`)
* appending to existing recording (`asciicam rec --append <filename>`)
* raw recording mode, storing only stdout bytes (`asciicam rec --raw <filename>`)
* environment variable white-listing (`asciicam rec --env="VAR1,VAR2..."`)
* toggling pause in `asciicam play` by <kbd>Space</kbd>
* stepping through a recording one frame at a time with <kbd>.</kbd> (when playback paused)
* new `asciicam cat <filename>` command to dump full output of the recording
...
* New `--speed` option for `asciicam play` (thanks Bastiaan "@bastiaanb" Bakker!)
* Ability to set API token via `ASCIICAM_API_TOKEN` env variable (thanks Samantha "@samdmarshall" Marshall!)
* Improved shutdown on more signals: CHLD, HUP, TERM, QUIT (thanks Richard "@typerlc"!)
* Fixed stdin handling during playback via `asciicam play`

## 1.3.0 (2016-07-13)

This release brings back the original Python implementation of asciicam. It's
based on 0.9.8 codebase and adds all features and bug fixes that have been
implemented in asciicam's Go version between 0.9.8 and 1.2.0.
...
* Added playback from stdin: `cat demo.json | asciicam play -`
* Added playback from IPFS: `asciicam play ipfs:/ipfs/QmcdXYJp6e4zNuimuGeWPwNMHQdxuqWmKx7NhZofQ1nw2V`
* Added playback from asciicast page URL: `asciicam play https://asciicam.org/a/22124`
...
* Import paths changed to `github.com/khulnasoft-lab/asciicam` due to repository
  renaming
...
* Recorded process has `ASCIICAM_REC` env variable set (useful for "rec"
...
* Introduce proper python package: https://pypi.python.org/pypi/asciicam
...
* Project rename from "ascii.io" to "asciicam"
